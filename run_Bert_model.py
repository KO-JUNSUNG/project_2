...
from data_cls import DataPrecessForClassification as DataSet
...
def model_train_validate_test(train_df, dev_df, test_df, target_dir, 
    ...
    train_data = DataSet(tokenizer, train_df, max_seq_len=max_seq_len)
    train_loader = DataLoader(train_data, shuffle=True, batch_size=batch_size)

    print("\t* Loading validation data...")
    dev_data = DataSet(tokenizer,dev_df, max_seq_len = max_seq_len)
    dev_loader = DataLoader(dev_data, shuffle=True, batch_size=batch_size)
    
    print("\t* Loading test data...")
    test_data = DataSet(tokenizer,test_df, max_seq_len = max_seq_len) 
...
def model_load_test(test_df, target_dir, test_prediction_dir, test_prediction_name, max_seq_len=256, batch_size=16):
    """
    Parameters
    ----------
    test_df : pandas dataframe of test set.
...
if __name__ == "__main__":
    train_df = pd.read_csv("./combined_data/combined_train.csv")
    dev_df   = pd.read_csv("./combined_data/combined_dev.csv")
    test_df  = pd.read_csv("./combined_data/combined_test.csv")
